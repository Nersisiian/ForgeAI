...
async def test_get_project_tasks_empty(client: AsyncClient, db_session):
    ...
    project = Project(
        name="P", natural_language_query="Q", target_type="fastapi", owner_id=user.id
    )
...
async def test_get_project_tasks_with_tasks(client: AsyncClient, db_session):
    ...
    task1 = GenerationTask(
        project_id=project.id, agent_type="planner", status="success"
    )
    task2 = GenerationTask(
        project_id=project.id, agent_type="backend", status="running"
    )
...
async def test_forbidden_task_access(client: AsyncClient, db_session):
    ...
    user1 = User(
        email="f1@t.com", hashed_password=get_password_hash("test"), is_active=True
    )
    user2 = User(
        email="f2@t.com", hashed_password=get_password_hash("test"), is_active=True
    )
