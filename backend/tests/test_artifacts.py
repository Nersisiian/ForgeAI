...
async def test_get_artifacts_empty(client: AsyncClient, db_session):
    ...
    response = await client.get(
        f"/api/v1/artifacts/project/{project.id}", headers=headers
    )
...
async def test_update_artifact(client: AsyncClient, db_session):
    ...
    await db_session.commit()
    artifact = FileArtifact(
        project_id=project.id, file_path="test.py", content="old", status="draft"
    )
    db_session.add(artifact)
    await db_session.commit()
    ...
    response = await client.put(
        f"/api/v1/artifacts/{artifact.id}", json=update_payload, headers=headers
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_unauthorized_artifact_access(client: AsyncClient, db_session):
    user1 = User(
        email="u1@t.com", hashed_password=get_password_hash("test"), is_active=True
    )
    user2 = User(
        email="u2@t.com", hashed_password=get_password_hash("test"), is_active=True
    )
    db_session.add_all([user1, user2])
    await db_session.commit()
    token1 = create_access_token({"sub": str(user1.id)})
    project = Project(
        name="P", natural_language_query="Q", target_type="fastapi", owner_id=user2.id
    )
    db_session.add(project)
    await db_session.commit()
