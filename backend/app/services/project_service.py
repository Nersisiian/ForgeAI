...
class ProjectService:
    ...
    async def create_project(self, data: ProjectCreateRequest, owner_id: UUID) -> ProjectResponse:
        project = Project(
            **data.model_dump(),
            owner_id=owner_id,
        )
    ...
    async def list_projects(self, user_id: UUID, page: int = 1, page_size: int = 20) -> ProjectListResponse:
        query = select(Project).where(Project.owner_id == user_id).order_by(Project.created_at.desc())
        ...
        projects = (await self.session.execute(query.offset((page-1)*page_size).limit(page_size))).scalars().all()
